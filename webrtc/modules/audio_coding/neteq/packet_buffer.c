    ...
    for (i = 0; i < buffer_inst->maxInsertPositions; i++) {
        ...
        if (buffer_inst->payloadLengthBytes[i] != 0) {
            ...
            if (codec_pos >= 0) {
                 ...
                 * returns last_duration or always computes the real duration
                 * without looking at last_duration. If an implementation really
                 * wanted to use last_duration to compute a changing duration,
                 * we would have to iterate through the packets in
